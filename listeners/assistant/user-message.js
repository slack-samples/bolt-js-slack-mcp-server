import { DEFAULT_SYSTEM_CONTENT, openai } from '../../llm/openai.js';
/**
 ...
 */
export const userMessage = async ({ client, context, logger, message, say, setTitle, setStatus }) => {
  /**
  ...
  try {
    // If the user has not installed the app, prompt them to do so.
    if (!context.userToken) {
      await say({
        blocks: [
          {
            type: 'rich_text',
            block_id: 'install_app_message',
            elements: [
              {
                ...
                elements: [
                  {
                    type: 'text',
                    text: 'Oh no! This feature requires the app to be installed. Please install it to this workspace to continue.',
                  },
                ],
              },
            ],
    ...
    /**
     * Set the status of the Assistant to give an indication of active processing.
     *
    ...
    // Send message history and newest prompt to LLM
    ...
    // Provide a streamed response to the user
